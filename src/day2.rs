...
impl Choice {
    fn from_string(x : &str) -> Choice {
        match x {
            ...
            "B"|"Y" => Choice::Paper,
            "C"|"Z" => Choice::Scissors,
            _ => { panic!("Invalid Choice input: {}", x) }
        }
    }
}
...
impl Outcome {
    fn from_string(x: &str) -> Outcome {
        match x {
            ...
            "Y" => Outcome::Draw,
            "Z" => Outcome::Win,
            _ => { panic!("Invalid Outcome input: {}", x) }
        }
    }
}
...
impl FromInput for Day2 {
    fn from_lines(lines: impl Iterator<Item = String>) -> Self {
        ...
        for l in lines {
            ...
            m.moves.push( Move { me : Choice::from_string(me), opponent: Choice::from_string(opp), goal : Outcome::from_string(me) } );
        }
        m
    }
}
